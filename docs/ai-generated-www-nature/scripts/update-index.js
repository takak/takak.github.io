...
<head>
  ...
  <title>Nature Garden - 成長する自然</title>
  <style>
    body {
      margin: 0;
      padding: 0;
...
<body>
  <div class="header">
    <h1>🌳 Nature Garden</h1>
    <div class="subtitle">毎日成長する自然 - ${days.length}日目</div>
  </div>
  ...
  <div class="info-panel">
    <h2 style="color: #6c6; margin-top: 0;">📊 現在の状態 - Day ${latestScene.metadata.dayNumber}</h2>
    <div class="stat">
      <span class="stat-label">日付</span>
      <span>${latestDay}</span>
    </div>
    <div class="stat">
      <span class="stat-label">季節</span>
      <span>${latestScene.metadata.season} (${latestScene.metadata.seasonEn})</span>
    </div>
    <div class="stat">
      <span class="stat-label">年</span>
      <span>Year ${latestScene.metadata.cycleYear}</span>
    </div>
    <div class="stat">
      <span class="stat-label">木</span>
      <span>${latestScene.permanentElements.trees.length}本</span>
    </div>
    <div class="stat">
      <span class="stat-label">岩</span>
      <span>${latestScene.permanentElements.rocks.length}個</span>
    </div>
    <div class="stat">
      <span class="stat-label">草</span>
      <span>${latestScene.seasonalElements.grasses.length}本</span>
    </div>
    <div class="stat">
      <span class="stat-label">花</span>
      <span>${latestScene.seasonalElements.flowers.length}輪</span>
    </div>
    <div class="stat">
      <span class="stat-label">生態系成熟度</span>
      <span>${(latestScene.ecosystem.maturityLevel * 10).toFixed(0)}/10</span>
    </div>
  </div>
